...
- [ ] Open question: Spec §5 specifies `gap_norm = |Δ₀| / reward_scale` with `reward_scale = max(R_max, |V*_{0,γ,0}(s₀)|, 1e-8)` but the §4 promotion gate lists both `contest_gap_norm ≤ 0.01` (strict) and `≤ 0.02` (with explicit justification) — who adjudicates the "explicit justification" for the soft band, and is it a per-task plotter-analyst note or a planner-level gate override? Current plan treats the soft band as exploratory and promotes only strict-band tasks unless otherwise directed.
- [ ] Open question: Spec §7 WP5 mandates "raw-unclipped only on safety-stress tasks" (Family C and its refinements). If the shortlist contains zero Family C tasks after the refinement loop, does WP4 / WP5 raw-arm execution become a no-op, or does the refinement loop continue until at least one Family C task is promoted even if `≥ 2 positive` are already found? Current plan assumes the §5 stopping rule (`≥ 2 positive + ≥ 1 safety`) is a conjunction, not a disjunction, so the loop continues until a Family C task promotes.
- [ ] Open question: Spec §7 WP1c sets a hard cap of `≤ 5,000 candidate configs aggregated across all families`. If WP1c + the full family-refinement loop (A/B/C/D/E) together exceed 5,000 candidates, is the cap per search-iteration or a lifetime budget? Current plan assumes per-iteration; if the intent is lifetime, the refinement loop needs a budget tracker.
- [ ] Open question: Spec §9 requires every runner to emit `experiment_manifest.json` "from within the runner, not post hoc". The existing Phase IV runners emit this post hoc from aggregators in several cases. Does the WP0 audit need to surface this as a drift finding for every Phase I–IV runner, or is the in-runner requirement forward-looking for Phase V only? Current plan treats it as forward-looking for Phase V runners; WP0 does not retroactively re-emit manifests for Phase I–IV.
