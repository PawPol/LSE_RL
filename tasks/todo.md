...
- [x] [follow-up] [run_phase3_dp.py / SafeVI / SafePE] WP0 audit (2026-04-23) flags `d_eff_bound` and `cert_bound` BLOCKERs on SafePE and SafeVI runs for `chain_catastrophe`, `chain_jackpot`, `grid_regime_shift{,_pre_shift,_post_shift}`, `grid_sparse_goal`. Per-stage `safe_effective_discount_mean` sits around 1.028 (not near γ=0.99) and exceeds the certified `kappa_t ≈ 0.99` by ~3.8e-02. **Remediated (2026-04-24, task #11) — downgraded to MINOR.** The grid-mean breach is an algebraic artifact: absorbing / terminal cells have `P[s, a, :] = 0`, so `E_{s'}[(1+γ)(1-ρ)]` collapses to `(1+γ)·1 = 1.99` on a single (s, a) row while the operator's target itself is 0 (no future value to discount). Audit now applies an occupancy-weighted cert check `d_eff_occ[t] = Σ_{s,a} d_ref(t,s)·π*_safe(a|t,s)·eff_d[s,a,t]` with absorbing cells masked from `d_ref`; every Phase III DP run passes. 76 BLOCKERs (`cert_bound` × 38 + `d_eff_bound` × 38) downgraded to MINOR. See `scripts/audit/occupancy_cert_check.py` and `results/audit/consistency_report.{md,json}`.
- [x] [follow-up] [run_phase4_dp.py / safe_vi] WP0 audit (2026-04-23) flags `cert_bound` BLOCKERs on Phase IV-B `safe_vi` runs on both `dense_chain_cost_0` and `dense_chain_cost_1`, all seeds. At stage 17 `safe_effective_discount_mean ≈ 0.996` exceeds `kappa_t ≈ 0.961` by ~3.4e-02 (ed < 1 so gate 1 passes, but the certified stagewise bound is violated). **Remediated (2026-04-24, task #11) — downgraded to MINOR.** Same occupancy-weighted recheck as Phase III: the per-(s,a) overshoot lives on the absorbing goal cell (zero P row ⇒ algebraic eff_d = 1+γ) and carries no `d_ref` mass. All 6 Phase IV-B `safe_vi` runs pass the occupancy-weighted cert on every stage. 6 BLOCKERs downgraded to MINOR.
- [ ] Open question (2026-04-24, task #11): the absorbing-cell mask that licenses the WP0 cert downgrades currently lives in `scripts/audit/occupancy_cert_check.py` only. WP1b's `experiments/weighted_lse_dp/search/candidate_metrics.py` computes §6 `delta_d`, `mass_delta_d`, and clip-fraction statistics under `d_ref` — if it does NOT apply the same absorbing `(s, a)` mask, it will inherit the grid-mean artifact (eff_d = 1+γ on zero-P cells) and could surface spurious activation in the search shortlist. Decision needed: factor a shared `absorbing_mask(mdp.p)` helper into `experiments/weighted_lse_dp/search/reference_occupancy.py` and call it from both the audit and the candidate-metrics module. Defer to WP1b planning.
...
- [ ] [env-builder] Open question (2026-04-24, WP2): the "certified_bound" used in the Family C stress-diagnostic test is currently taken as `κ_t = gamma` (classical contraction ceiling, stationary schedule); the spec phrases the bound as "above the safe certificate" without a stage-by-stage κ_t formula — confirm with operator-theorist whether `max(kappa_t)` from the certified schedule should replace this floor in WP1c's production gate.
- [ ] [env-builder] Open question (2026-04-23, WP2, Family D): the orchestrator prompt's literal shallow_early_warning formula `lam_tie = p_warn·(γ^(L−1−warning_lead)·r_shallow − p_cat·γ^(L−1)·C)` is off-sign relative to the prompt's "negative of nominal expected return" description. Shipped the sign-consistent form `lam_tie = p_warn·(p_cat·γ^(L−1)·C − γ^(L−1−warning_lead)·r_shallow)` (so `lam ≥ 0` for `r_shallow = 0.1·C` at every admissible ψ — preventive cost must be non-negative). Confirm with orchestrator that the sign-consistent form is intended before the Phase V registry wiring lands.
- [ ] [env-builder] Open question (2026-04-24, WP2, Family E task #15): the orchestrator prompt's literal E1 tie formula `lam_tie = (1-γ^L)/(1-γ)·r_smooth − p_warn·γ^{t_cat}·C_conc` equals `E[ignore]`, not `-E[ignore]`. Since `V(react, s_start) = -lam_tie` and the tie requires `V(react) = V(ignore)`, we need `lam_tie = -E[ignore] = p_warn·γ^{t_cat}·C_conc − (1-γ^L)/(1-γ)·r_smooth`. Shipped the sign-consistent form (exact-DP residual `|Δ₀| ≤ 1e-8` verified in `tests/families/test_family_e.py::test_E1_tie`); E2 and E3 prompt formulas already match. Same precedent as the Family D open question — confirm with orchestrator before Phase V registry wiring.

### WP1c — search driver + shortlist — depends on WP1 (metric module, tie solver) + WP2 (families A/B/C)

- [x] [experiment-runner] [infra] Implement `experiments/weighted_lse_dp/runners/run_phase_V_search.py` replacing the legacy `run_phase4_activation_search.py` (retained with module-level `DeprecationWarning`); driver applies a cheap `contest_gap_norm` prefilter before full DP, enforces the `≤ 5,000 candidate configs` aggregated hard cap, writes `experiment_manifest.json` from within the runner (spec §7 WP1c, §9). **Landed 2026-04-24.**
- [x] [experiment-runner] [infra] Emit search outputs `results/search/{candidate_grid,candidate_metrics,phase_diagram_data}.parquet` + `results/search/near_indifference_catalog.csv` with schema headers (spec §7 WP1c, §8 directory layout). **Landed 2026-04-24.**
- [x] [experiment-runner] [infra] Apply the §4 promotion gate to compute `results/search/shortlist.csv` and `results/search/shortlist_report.md`; all thresholds hard-coded and logged (spec §4, §7 WP1c). Do not optimize the shortlist for RL return. **Landed 2026-04-24.**
- [x] [experiment-runner] [infra] Empty-shortlist contract: when shortlist is short, emit `results/search/shortlist_refinement_manifest.md` listing which levers (tie / geometry / delay / concentration / warning depth / asymmetry) to refine; do not relax thresholds (spec §7 WP1c empty-shortlist contract, §4 "report that internally, refine families, and rerun"). Blocks WP3/WP4/WP5 if `< 2 promotable positive families` or `< 1 safety family`. **Landed 2026-04-24.**
- [x] [test-author] [test] Integration test for the search driver on a 50-candidate synthetic grid: promotion gate yields deterministic shortlist, `contest_gap_norm` prefilter reduces DP calls by ≥ 50%, and the refinement manifest is emitted iff shortlist is short (spec §4, §7 WP1c). **Landed 2026-04-24** — `tests/runners/test_run_phase_V_search.py` 6 tests pass (smoke, prefilter, gate, empty-shortlist, cap, CLI).
- [ ] [experiment-runner] Open question: the spec §7 WP1c line "each family must expose `family.psi_grid()` returning an iterable of ψ dicts" is not implemented on `FamilySpec` (which WP1a explicitly froze); the WP1c runner instead owns grid enumeration from the config's `family_params` block — confirm this is the intended split, or WP1a/WP2 must be amended to add the method.
...
- [x] [algo-implementer] [algo] Implement limited-`k`-backup finite-horizon planner that records `||V_k − V*||_∞` per stage and per distance-to-terminal, greedy-action correctness rate per stage, earliest stage at which the correct action is recovered, backward propagation distance of a rare reward or warning signal after `k` backups, occupancy-weighted value error under `d_ref`, and policy-disagreement trajectory (spec §7 WP3 items 1–6). <!-- Landed 2026-04-24: `experiments/weighted_lse_dp/runners/run_phase_V_planning.py`. Reuses `_classical_qv`/`_safe_qv`/`_safe_q_stage` from `search.candidate_metrics` and `_calibrate_schedule` from `run_phase_V_search`; no algorithmic reimplementation. -->
- [x] [experiment-runner] [infra] Run the limited-backup diagnostic across every shortlisted task for safe and classical operators, persist to `results/planning/limited_backup_metrics.parquet` with schema header (spec §7 WP3 data deliverable). Depends on the limited-backup planner and shortlist. <!-- Wall-clock 0.61 s on 5 A tasks, horizon up to 8; parquet + scalars parquet emitted; manifest written to results/summaries/experiment_manifest.json. 2026-04-24. -->
- [x] [test-author] [test] Unit test on a toy MDP with an analytically known `V*`: the limited-backup planner's `||V_k − V*||_∞` decays monotonically for classical VI and policy-disagreement trajectory converges to zero as `k → T` (spec §7 WP3 item 6). <!-- tests/runners/test_run_phase_V_planning.py (4 tests; pytest -v → 4 passed in 1.31s). -->
- [x] [plotter-analyst] [plot] Build `figures/main/fig_propagation_curve.pdf` — safe vs classical propagation distance of the rare reward/warning signal after `k` backups on the lead aligned-propagation shortlisted task (spec §7 WP6 main figure 3). <!-- Emitted by analysis/make_phaseV_wp3_figures.py; lead task = A_003 (|value_gap_norm| = 7.82e-3). -->
- [x] [plotter-analyst] [plot] Build `figures/main/fig_stagewise_error_heatmap.pdf` — `||V_k − V*||_∞` heatmap indexed by (stage, backup count `k`) for safe and classical planners (spec §7 WP3 required plots). <!-- Emitted alongside fig_greedy_recovery.pdf. -->
- [ ] [algo-implementer] Open question: On the current Family A shortlist (all `promotion_mode = safe_active_no_distortion`), classical and safe `||V_k - V*_own||_∞` at `t=0` are numerically identical at every `k` — the safe operator essentially collapses to classical on these tasks. Is the main-paper planning story strong enough with these 5 tasks alone, or does WP3 need a Family A refinement pass that forces `binding_clip` to produce a visible propagation contrast?
...
## Phase VII — Adaptive-β experiments (2026-04-26)

Source spec: `docs/specs/phase_VII_adaptive_beta.md`. Coarse-spec input:
`tasks/adaptive_beta_experiments_coding_agent_spec.md`. Phase role:
**exploratory only**. The pruned main-paper §Experiments (commit `ccec6965`)
is not touched in this phase. Final deliverable is a report plus
recommendation memo; the user — not the planner, not the runner — decides
whether the results land in appendix / supplementary / future revision.

Stage gating: A → B → C, each gated on explicit user sign-off. Codex review
gates land at the end of M4 and M5 only; M1–M3 use light role-agent +
verifier review.

### M1 — operator import + adaptive-β schedule

1.  - [ ] [spec-read] Re-read Phase VII spec §0–§4 + §22 and confirm scope, decisions, and Open questions before any implementation → planner  (spec §0–§4, §22)
2.  - [x] [operator] op-import: ~~resolve operator import surface~~ **RESOLVED 2026-04-26 — option (b)**. Phase VII spec §3.4 / §22.1 now mandates extracting the stateless LSE/TAB kernel (lines 459–480 of `mushroom-rl-dev/mushroom_rl/algorithms/value/dp/safe_weighted_common.py`) into `src/lse_rl/operator/tab_operator.py` and rewiring `SafeWeightedCommon` to import from it. No further decision work required.
3.  - [ ] [operator] op-extract: extract the stateless LSE/TAB kernel into `src/lse_rl/operator/tab_operator.py` exposing `g(beta, gamma, r, v)`, `effective_discount(beta, gamma, r, v)`, and `rho(beta, gamma, r, v)` with the β=0 collapse exactly matching `r + γv`. Refactor `mushroom_rl/algorithms/value/dp/safe_weighted_common.py` so that `compute_safe_target`, `compute_effective_discount`, and `compute_rho` import from the new module without behavioral change. Acceptance: import succeeds from both call sites; `SafeWeightedCommon` retains identical public API; `g(0,γ,r,v) == r+γv` exact; mushroom-rl-dev edit recorded in `tasks/lessons.md` per CLAUDE.md §4 → operator-theorist  (spec §3.1–§3.4, §22.1)
3a. - [ ] [test] op-equivalence: regression/property tests verifying numerical equivalence (a) before vs after the refactor for `SafeWeightedCommon.compute_safe_target` over a fixed `(β, γ, r, v)` grid, and (b) between the new shared kernel and the old inlined logaddexp branch. Acceptance: pytest green; tests fail loudly if either side drifts → test-author  (spec §22.1 constraints, test 13.1.6)  Blocked by op-extract.
4.  - [ ] [scheduler] schedules: implement `experiments/adaptive_beta/schedules.py` with `AdaptiveBetaSchedule`, `FixedBetaSchedule`, `ZeroBetaSchedule`, `WrongSignSchedule`, `AdaptiveSignOnlySchedule`, `AdaptiveMagnitudeOnlySchedule`. β updated between episodes only via `A_e = mean(r_t - v_next_t)`; raw rule `β_max·tanh(k·A_e)`; symmetric clip; optional smoothing `λ`. Acceptance: schedule object exposes `beta_for_episode(e)` and `update_after_episode(e, transitions)` and is unit-testable in isolation → calibration-engineer  (spec §4)
5.  - [ ] [test] Operator tests under `tests/adaptive_beta/test_operator.py`: β→0 recovery, finite-difference derivative vs `effective_discount`, alignment condition on a 100-pt grid, log-sum-exp stability at extreme inputs, numerical agreement with `SafeWeightedCommon` (spec test 13.1.6). Acceptance: pytest green → test-author  (spec §13.1)  Blocked by op-import.
6.  - [ ] [test] Schedule tests under `tests/adaptive_beta/test_schedules.py`: β constant within episode (spy-based), β for episode `e+1` reads only data ≤ `e` (forbidden-future spy), clip never exceeds `β_cap` for `adaptive_beta`, no-clip variant exceeds cap on a divergence-prone synthetic trace and sets `divergence_event=True`. Acceptance: pytest green → test-author  (spec §13.2 + §13.5)  Blocked by `schedules`.

### M2 — environments

7.  - [x] [env] env-base-decision: ~~resolve Open question §22.2~~ **RESOLVED 2026-04-26 — MushroomRL `Environment` subclass.** Phase VII spec §6 / §22.2 now mandates Phase I–VI precedent. Implementation tasks below must subclass `mushroom_rl.core.environment.Environment`, handle `(state, info)` from `reset()`, and use `int(np.asarray(x).flat[0])` for state-scalar normalization. Plain Python helpers allowed only as internal collaborators (e.g. opponent controllers). Record the decision in `experiments/adaptive_beta/envs/__init__.py` module docstring when the package is created.
8.  - [ ] [env] Implement `envs/rps.py` as a `mushroom_rl.core.environment.Environment` subclass (adversarial RPS) with hidden-phase + visible-phase variants, switch_period_episodes ∈ {50, 100}, oracle best-response, mandatory `info` dict keys. `reset()` returns `(state, info)`; numpy state-scalar normalization via `int(np.asarray(x).flat[0])`. **`wrong_sign` is not defined for this env** (no canonical sign per §22.3). Acceptance: `reset(seed)` deterministic; phase switches at correct episodes; rewards match spec → env-builder  (spec §6.1, §22.2, §22.3)
9.  - [ ] [env] Implement `envs/switching_bandit.py` as a MushroomRL `Environment` subclass (5-arm Bernoulli, horizon=1, switch every {100,250} episodes). Log per-episode regret vs current best arm. Env docstring must record the §22.5 resolution: mechanism degenerate at H=1 ⇒ env contributes to regret/AUC/recovery only, **excluded from alignment-rate and effective-discount mechanism panels** and from mechanism columns of the main results table. **`wrong_sign` is not defined for this env** (§22.3). Acceptance: same-seed reproducibility; correct best-arm rotation; docstring documents the mechanism exclusion → env-builder  (spec §6.2, §22.2, §22.3, §22.5)
10. - [ ] [env] Implement `envs/hazard_gridworld.py` as a MushroomRL `Environment` subclass (7×7, horizon=50, 5 hazards, switch every {100,250}). Catastrophe flag fires on hazard entry. Hazards lie on plausible shortest-path corridors. **Canonical sign for `wrong_sign` ablation: −β; `wrong_sign` here = +β** (spec §6.3 + §22.3). Acceptance: terminals work (goal/hazard/horizon); hazard layout reproducible per seed; canonical sign exposed via env metadata so the schedule object can validate → env-builder  (spec §6.3, §22.2, §22.3)
11. - [ ] [env] Implement `envs/delayed_chain.py` as a MushroomRL `Environment` subclass (chain_length=20, horizon=25, terminal=+50, distractor=`reset`). Oracle action = `forward`. **Canonical sign for `wrong_sign` ablation: +β; `wrong_sign` here = −β** (spec §6.4 + §22.3). Acceptance: `forward × chain_length` produces terminal reward; reset action observed; canonical sign exposed via env metadata → env-builder  (spec §6.4, §22.2, §22.3)
12. - [ ] [env] Implement `envs/selfplay_rps.py` as a MushroomRL `Environment` subclass (two-agent symmetric + asymmetric A=adaptive/B=vanilla). Outputs exploitability proxy, action entropy, cycling indicator, return variance. **Stage-A excluded; Stage-B fresh entry, gated on Stage A signal on at least one of the four primary envs (§22.4).** **`wrong_sign` is not defined for this env** (§22.3). Acceptance: two-agent runner is callable from the standard runner; env docstring records the §22.4 promotion rule → env-builder  (spec §6.5, §22.2, §22.3, §22.4)
13. - [ ] [test] Env tests under `tests/adaptive_beta/test_envs.py`: phase switches at correct episodes, rewards match spec, terminal conditions fire, same seed reproduces same env sequence (action-by-action equality) for all five envs. Acceptance: pytest green → test-author  (spec §13.3)  Blocked by env tasks 8–12.

### M3 — agent + training loop + dev run

14. - [ ] [algo] agent: implement `experiments/adaptive_beta/agents.py` Q-learning with one shared update routine consuming a schedule object — supports `vanilla / fixed_positive / fixed_negative / wrong_sign / adaptive_beta / adaptive_beta_no_clip / adaptive_sign_only / adaptive_magnitude_only`. β=0 path is exactly `r + γv`. ε-greedy schedule shared across methods. Operator math is imported from `src/lse_rl/operator/tab_operator.py` (per §22.1 / task 3). **`wrong_sign` schedule is wired only for `delayed_chain` (canonical +β → wrong −β) and `hazard_gridworld` (canonical −β → wrong +β); construction must raise on any env without canonical-sign metadata (RPS, switching bandit, self-play RPS).** Acceptance: runtime assertion that fixed-β methods and adaptive methods enter the same update function; `vanilla` produces classical TD targets bit-identical to a baseline classical Q-learning impl; `wrong_sign(rps)` etc. raise → algo-implementer  (spec §5, §16.1–§16.2, §22.1, §22.3)  Blocked by `op-extract` (task 3), `schedules` (task 4), env tasks 8–11.
15. - [ ] [logging] Implement logging callbacks emitting episode CSV + transition CSV/parquet with the exact schema headers from spec §7.4 / coarse-spec §15. Includes `divergence_event`, `nan_count`, `q_abs_max`, `td_target_abs_max`. Acceptance: a 100-episode dev run writes both files with all required columns and a schema header → experiment-runner  (spec §7, §15)  Blocked by `agent`.
16. - [ ] [infra] Build `run_experiment.py` with `--config` (yaml) + `--seed` + `--out`. Emit `run.json` (git SHA, argv, seed list, task/agent/schedule config, timestamps, output paths) + `metrics.npz` (schema header) + transitions parquet. Append entry into `results/summaries/experiment_manifest.json`. Acceptance: a single dev run produces all four artifacts and the manifest line → experiment-runner  (spec §15, CLAUDE.md §4)  Blocked by `agent`, logging.
17. - [ ] [infra] Configs: write `configs/dev.yaml` (Stage A — 5 envs × 5 methods × 3 seeds × 1k eps, no sensitivity), `configs/main.yaml` (Stage B template — 1–2 envs × 8 methods × 10 seeds × 10k eps, ablations on), `configs/headline.yaml` (Stage C — 1 env × core methods × 20 seeds × 10k eps), `configs/ablations.yaml` (sensitivity grid). Acceptance: every config validates against a config schema and references env/method names that exist → experiment-runner  (spec §8.1–§8.3, §11)
18. - [ ] [test] Reproducibility test under `tests/adaptive_beta/test_reproducibility.py`: run `dev.yaml` with seed=11 twice and assert byte-identical `metrics.npz` (after sorting nondeterministic dict keys). Acceptance: pytest green → test-author  (spec §13.4)  Blocked by tasks 14–17.
19. - [ ] [test] Same-code-path test: assert that `vanilla`, `fixed_positive`, `adaptive_beta`, `adaptive_beta_no_clip` all enter the same `_step_update` function and differ only in the schedule object (e.g. via a function-call counter shared across methods). Acceptance: pytest green → test-author  (spec §16.2)  Blocked by `agent`.
20. - [ ] [infra] Stage A dev run: dispatch the **60-run dev matrix (4 envs × 5 methods × 3 seeds)** — envs = {`rps`, `switching_bandit`, `hazard_gridworld`, `delayed_chain`}, methods = {`vanilla`, `fixed_positive`, `fixed_negative`, `adaptive_beta`, `adaptive_beta_no_clip`}. **Self-play RPS is excluded from Stage A per §22.4.** Aggregate per-(env, method) summaries into `results/adaptive_beta/processed/`. Acceptance: every run completes or the failure is logged in the manifest → experiment-runner  (spec §8.1, §22.4)  Blocked by tasks 14–18.

### M3.5 — Stage A gate

21. - [ ] [infra] Stage A summary: produce `results/adaptive_beta/stage_A_summary.md` listing per-env paired-mean improvement of `adaptive_beta` over `vanilla` on AUC + recovery time, with mechanism evidence (alignment rate on informative transitions, mean d_eff vs γ) **for envs other than `switching_bandit` (§22.5: bandit is performance-only — report regret/AUC/recovery; alignment-rate and `d_eff` are not produced)**. Identify candidate 1–2 strongest envs for Stage B promotion. The summary must explicitly state whether at least one of the four Stage-A envs cleared the §22.4 promotion bar (stable adaptive-β signal + controlled variance) — this controls whether `selfplay_rps` is added in Stage B. Acceptance: summary exists; user has explicitly named the promoted environments and ruled on self-play inclusion (durable record in `tasks/todo.md`). **Blocks all M4 work.** → experiment-runner + plotter-analyst  (spec §8.1 acceptance, §16 item 9, §22.4, §22.5)

### M4 — Stage B main runs + figures + tables + ablations

22. - [ ] [infra] Stage B main runs: 1–2 promoted envs × 8 methods × 10 seeds × 10k eps. Write to `results/adaptive_beta/raw/` and aggregate to `processed/`. Acceptance: manifest accounts for every run; no silent drops → experiment-runner  (spec §8.2)  Blocked by Stage A gate (task 21).
23. - [ ] [ablation] Sensitivity grid (Stage B only, strongest 1–2 envs): `β_max ∈ {0.5,1,2}`, `k ∈ {1,5,10}`, `β_cap ∈ {0.5,1,2}` per coarse-spec §11.2 priority. No full Cartesian on every env. Acceptance: ablation table populated → experiment-runner  (spec §11.2)  Blocked by task 22.
24. - [ ] [ablation] Difficulty-knob sweep (strongest env only): vary shift frequency, reward noise, hazard density, chain length to characterize where the mechanism activates. Acceptance: per-knob mechanism-metric trajectories archived → experiment-runner  (spec §11.3)  Blocked by task 22.
25. - [ ] [plot] Per-env figures (1–2 promoted envs): `learning_curves_{env}.pdf`, `regime_shift_recovery_{env}.pdf`, `alignment_rate_{env}.pdf`, `effective_discount_{env}.pdf` (with γ guide line), `beta_trajectory_{env}.pdf`, `advantage_histogram_{env}.pdf`. **`alignment_rate_*` and `effective_discount_*` are NOT produced for `switching_bandit`** (§22.5 — mechanism degenerate at H=1). Acceptance: every figure regenerable from `results/adaptive_beta/processed/` → plotter-analyst  (spec §10.1, §22.5)  Blocked by task 22.
26. - [ ] [plot] Killer figure: `adaptive_beta_mechanism_summary.pdf` — 4 panels (post-shift learning curve, recovery-time bar, alignment-rate trajectory, γ−d_eff on informative transitions). **Mechanism panels (alignment, γ−d_eff) must be drawn from envs other than `switching_bandit`** per §22.5; if bandit is the headline performance env, the mechanism panels still come from a non-bandit env and the figure caption notes the split. Acceptance: figure exists; passes plotter visual check → plotter-analyst  (spec §10.2, §22.5)  Blocked by task 22.
27. - [ ] [analysis] Statistical reporting: paired-bootstrap 95% CIs (10k resamples) for `adaptive_beta` vs `vanilla`, vs best fixed-β, vs `adaptive_beta_no_clip`. Save numeric results to `results/adaptive_beta/processed/paired_differences.parquet`. Acceptance: every CI uses paired seeds; unpaired comparisons are flagged → plotter-analyst  (spec §9)  Blocked by task 22.
28. - [ ] [plot] Tables: main results, ablation, sensitivity. Save as both `.csv` and `.tex` under `results/adaptive_beta/tables/`. Every table includes `align_rate` and `mean_d_eff` columns (spec §16 item 5); **for `switching_bandit` rows those two cells are reported as `n/a — degenerate at H=1`** (§22.5). Acceptance: schema enforced; bandit cells correctly labeled → plotter-analyst  (spec §10.3, §22.5)  Blocked by task 27.
29. - [ ] [infra] Stage B Codex gate (full review per AGENTS.md §5.2): commit closing branch; `/codex:review --base main --background`; `/codex:adversarial-review --base main --background "<spec §14.1 Phase VII focus string>"`; `review-triage` writes BLOCKER/MAJOR/MINOR/NIT to `tasks/todo.md`; resolve all BLOCKERs; rerun `verifier`. Acceptance: BLOCKER == ∅; verifier green → verifier + review-triage  (spec §14.1, AGENTS.md §5.2)  Blocked by tasks 22–28.
30. - [ ] [infra] Stage B summary: `results/adaptive_beta/stage_B_summary.md` declaring whether the headline `adaptive_beta` vs `vanilla` paired-CI excludes 0 on at least one env, mechanism is active, and naming the candidate Stage C headline env. **Blocks Stage C decision.** → plotter-analyst  (spec §8.2 acceptance)  Blocked by task 29.

### M4.5 — Stage C headline (conditional)

31. - [ ] [infra] Stage C headline run: 1 env × {vanilla, fixed_positive, fixed_negative, adaptive_beta, adaptive_beta_no_clip} × 20 seeds × 10k eps **iff** Stage B effect is strong + stable per spec §8.2. Otherwise document the skip in `stage_C_summary.md` with reasoning. Acceptance: either run completes with manifest, or skip is recorded → experiment-runner  (spec §8.3)  Blocked by Stage B gate (task 30).

### M5 — final report + Codex gate

32. - [ ] [analysis] Write `results/adaptive_beta/final_report.md` per spec §17: §1 verdict (support / partial / fail) with paired-CI numbers, setup, results, mechanism diagnostics, ablations, failure cases, recommendation memo (appendix vs supplementary vs future revision — user decides), open implementation questions. Acceptance: §1 verdict explicit; recommendation memo distinct from a paper rewrite → plotter-analyst  (spec §17, §16 item 8)  Blocked by tasks 22–31.
33. - [ ] [infra] Final Codex gate (full review per AGENTS.md §5.2): `/codex:review --base main`; `/codex:adversarial-review --base main --background "<spec §14.1 Phase VII focus string>"`; triage; resolve BLOCKERs; rerun `verifier`. Acceptance: BLOCKER == ∅; verifier green → verifier + review-triage  (spec §14.1, §16)  Blocked by task 32.
34. - [ ] [audit] Verifier final audit: confirm (a) no commits in `paper/` attributable to Phase VII (spec §16 item 10), (b) every (env, method, seed) triple in every stage accounted for in `experiment_manifest.json` (spec §16 item 7), (c) `final_report.md` §1 verdict is explicit, (d) all spec §16 acceptance criteria pass. Acceptance: audit memo signed off → verifier  (spec §16)  Blocked by task 33.

### Dependencies (blocking edges)

- Stages: **A → B → C**, each blocked on user sign-off (tasks 21 → 22, 30 → 31).
- Milestones: **M1 → M2 → M3 → M3.5 → M4 → M4.5 → M5**.
- Within M1: `op-import` (task 2) blocks `agent` (task 14) and operator tests (task 5); `schedules` (task 4) blocks schedule tests (task 6) and `agent`.
- Within M2: `env-base-decision` (task 7) blocks env tasks 8–12; env tasks block env tests (task 13) and `agent` (task 14).
- Within M3: `agent` blocks logging, runner, reproducibility test, same-code-path test, dev run; logging schema is fixed in tasks 15–16 and is a prerequisite for every plot/table/aggregator task in M4 and M5.
- Stage A dev run (task 20) blocks Stage A summary (task 21).
- Stage A gate (task 21 + user sign-off) blocks all of M4.
- M4 main runs (task 22) block M4 ablation/plot/analysis/table tasks (23–28).
- M4 Codex gate (task 29) blocks Stage B summary (task 30) and the Stage C decision.
- Stage B summary (task 30) blocks Stage C run (task 31).
- M4 outputs + M4.5 outcome block M5 final report (task 32).
- Final Codex gate (task 33) blocks verifier audit (task 34); audit closes the phase.

### Parallelizable groups (worktree dispatch)

- **Group P1** (after spec re-read, before any code): `op-import` (task 2) and `env-base-decision` (task 7) can run in parallel as decision worktrees (no code edits, just spec resolution).
- **Group P2** (after Group P1 + `op-import` + `schedules`): operator tests (task 5), schedule tests (task 6), and the four single-env factories (tasks 8–11) — six independent worktrees. Self-play RPS (task 12) is gated on §22.4.
- **Group P3** (after `agent` lands): logging callbacks (task 15) and runner scaffolding (task 16) can run in parallel; env tests (task 13) can run alongside.
- **Group P4** (after Stage B main runs, task 22): per-env figures (task 25), killer figure (task 26), statistical reporting (task 27), table generation (task 28), ablation runs (tasks 23, 24) — five independent worktrees.
- **Group P5** (after final report drafted): the Codex review job and the adversarial-review job run in parallel (`--background`); poll with `/codex:status`.

### Open questions — RESOLVED 2026-04-26

All five §22 items locked by user (`tasks/phase_VII_clarifications.md` +
`tasks/phase_VII_confirmation.md`); spec §3.4 / §5 / §6.x / §8.1 / §10 /
§22 / §23 updated; tasks 2, 3, 7–12, 14, 20, 21, 25–28 rewritten to match.

- [x] §22.1 — **Operator import surface.** Option **(b)**: extract the stateless LSE/TAB kernel into `src/lse_rl/operator/tab_operator.py`; rewire `SafeWeightedCommon` to import from there. Refactor must be isolated, behavior-preserving, and pinned by an equivalence regression test (task 3 + 3a). MushroomRL-dev edit recorded in `tasks/lessons.md`.
- [x] §22.2 — **Environment base class.** MushroomRL `Environment` subclass for all five Phase VII envs; `(state, info)` from `reset()`, `int(np.asarray(x).flat[0])` for state-scalar normalization. Plain Python only as internal helpers. (Tasks 7–12 rewritten.)
- [x] §22.3 — **Definition of `wrong_sign` per environment.** Defined only on `delayed_chain` (canonical +β → wrong −β) and `hazard_gridworld` (canonical −β → wrong +β). For `rps`, `switching_bandit`, `selfplay_rps` use `fixed_positive` / `fixed_negative` instead; the schedule object must raise on construction otherwise. (Tasks 8–12, 14 rewritten.)
- [x] §22.4 — **Self-play RPS in Stage A.** Excluded from Stage A. Stage A matrix is 4 envs × 5 methods × 3 seeds = 60 runs. Self-play enters Stage B fresh, gated on at least one of the four Stage-A envs clearing the Stage-A → Stage-B promotion bar (stable adaptive-β signal + controlled variance). (Tasks 12, 20, 21 rewritten.)
- [x] §22.5 — **Bandit advantage at horizon = 1.** Mechanism degenerate at H=1. `switching_bandit` is kept for performance benchmarking only (regret, AUC, recovery, final return) and excluded from `alignment_rate_*` / `effective_discount_*` figures and from mechanism columns of the main results table (cells reported as `n/a — degenerate at H=1`). (Tasks 9, 25, 26, 28 rewritten.)

---

## Phase VII-B Strategic Learning (branch: phase-VII-B-strategic-2026-04-26)

Source spec: `tasks/phase_VII_B_strategic_learning_coding_agent_spec.md` (§§0–23).
Parent spec: `docs/specs/phase_VII_adaptive_beta.md`. Parent-phase outcome:
`results/adaptive_beta/final_recommendation.md` (PARTIAL SUPPORT on RPS only).

Branch invariants (locked 2026-04-26):
- Branch `phase-VII-B-strategic-2026-04-26` is already cut off `phase-VII-overnight-2026-04-26`.
- Full autonomous run authorized through Stage B2-Main + review gate + final memo.
- Compute budget ≤ ~20 min wall-clock through Main.
- Operator math is imported from `src/lse_rl/operator/tab_operator.py` — no duplication (spec §2.1, §12.2).
- Reuse `experiments/adaptive_beta/run_experiment.py`, `schedules.py`, `agents.py`, `logging_callbacks.py` (spec §4 last paragraph).
- Self-play (§7.10), Soda game (§6.6), Stage B2-Stress (§11.3) DEFERRED — not in Dev or Main matrices.
- Paper edits forbidden in this phase; only `paper_update/*.md` proposals (spec §17 + branch directive).
- Review gate is `/lse:review`-style background gate (NOT user-only `/codex:*`).

Orchestrator high-level slots (already created via TaskCreate as #1–#13):
- #1 spec read; #2–#6 implementation block; #7 verifier-after-impl; #8 Stage B2-Dev;
  #9 verifier-after-Dev + Dev→Main promotion; #10 Stage B2-Main; #11 review gate;
  #12 final memo + paper-update proposal; #13 close-out audit.
The numbered tasks below expand TaskCreate #2–#6 into atomic worktree units and
slot the gate/promotion/memo tasks (#7, #9, #11–#13) at correct boundaries.

### M0 — spec read + decision recap (TaskCreate #1)

VII-B-1.  - [ ] [spec-read] Re-read VII-B spec §§0–23 + §20 implementation order + §21 priorities; confirm scope vs DEFERRED list (self-play §7.10, Soda §6.6, Stage B2-Stress §11.3); record a one-page decision recap at `tasks/phase_VII_B_planner_recap.md` (no code) → planner  (spec §0–§3, §20–§21)

### M1 — package scaffold + history + reuse audit (expands TaskCreate #2)

VII-B-2.  - [ ] [infra] Create `experiments/adaptive_beta/strategic_games/` package skeleton: `__init__.py` (with §22.1 / §2.1 reuse note + branch identifier), empty stubs for `matrix_game.py`, `registry.py`, `history.py`, `metrics.py`, `games/__init__.py`, `adversaries/__init__.py`, `configs/`, `analysis/`. Acceptance: `python -c "import experiments.adaptive_beta.strategic_games"` succeeds; no game/adversary code yet → experiment-runner  (spec §4)
VII-B-3.  - [ ] [audit] Reuse audit memo `experiments/adaptive_beta/strategic_games/REUSE.md` listing exact import targets from `tab_operator.py`, `schedules.py`, `agents.py`, `run_experiment.py`, `logging_callbacks.py`. Flag any spec field (§7.4 / §13 logging schema) not already emitted by current logger and propose minimal additive fields. No code edits. Acceptance: memo committed; orchestrator can decide additive vs forked logger from it → planner  (spec §2.2, §4 last paragraph, §13)
VII-B-4.  - [ ] [infra] Implement `history.py`: `GameHistory` dataclass with `agent_actions / opponent_actions / agent_rewards / opponent_rewards / info` lists; methods `last(m)`, `empirical_agent_policy(m)`, `empirical_opponent_policy(m)`, `rolling_return(m)`. Pure NumPy/Python; no environment dependence. Acceptance: import succeeds; methods return correctly shaped arrays on a hand-crafted history → experiment-runner  (spec §5.3)
VII-B-5.  - [ ] [test] Unit tests `tests/adaptive_beta/strategic_games/test_history.py`: empirical-policy shape and normalization, `last(m)` slicing correctness, `rolling_return(m)` matches a NumPy reference on a 50-step synthetic trace. Acceptance: pytest green → test-author  (spec §12.1 finite-memory window correctness)  Blocked by VII-B-4.

### M2 — matrix-game environment (expands TaskCreate #3)

VII-B-6.  - [ ] [env] Implement `matrix_game.py`: `MatrixGameEnv` as a `mushroom_rl.core.environment.Environment` subclass per Phase VII §22.2 (locked decision). Constructor signature per spec §5.1; `reset()` returns `(state, info)`; state-scalar normalization via `int(np.asarray(x).flat[0])`; `current_phase()` and `info()` exposed. Default information regime: payoff/action-observing, model-hidden. Acceptance: `reset(seed=k)` deterministic; `step(a)` returns `(next_state, reward, absorbing, info)` with the spec §5.1 keys → env-builder  (spec §5.1)
VII-B-7.  - [ ] [env] Implement `registry.py`: registry mapping `game_name × adversary_name → factory(MatrixGameEnv)` with metadata block (action set, payoff matrices, supported adversaries, canonical-sign flag for `wrong_sign` if defined). Refuse unknown pairs with a loud `KeyError`. Acceptance: registry round-trips a (game, adversary) lookup; raises on unknown pairs → env-builder  (spec §4, §8 wrong_sign rules)
VII-B-8.  - [ ] [test] Unit tests `tests/adaptive_beta/strategic_games/test_matrix_game.py`: payoff-matrix indexing on a 3×3 fixture, env reset determinism (same seed → same state stream), env step determinism (same seed + same action sequence → same reward stream), `info()` contains spec §5.2 keys. Acceptance: pytest green → test-author  (spec §12.1)  Blocked by VII-B-6.

### M3 — adversaries (expands TaskCreate #4)

Within M3, adversary tasks are independent worktrees (no inter-adversary deps);
they all depend on `base.py` (VII-B-9) and `history.py` (VII-B-4).

VII-B-9.   - [ ] [env] Implement `adversaries/base.py`: `StrategicAdversary` ABC per spec §5.2 with `reset / act / observe / info` and the mandatory metadata block (`adversary_type, phase, memory_m, inertia_lambda, temperature, model_rejected, search_phase, hypothesis_id, policy_entropy`). Acceptance: ABC enforces the four methods and the `info()` schema → env-builder  (spec §5.2)
VII-B-10.  - [ ] [env] Implement `adversaries/stationary.py`: `StationaryMixedOpponent(probs)` — fixed action distribution, deterministic given seed. Acceptance: chi-square sanity check on a 10k-draw sample matches `probs` within tolerance → env-builder  (spec §7.1)
VII-B-11.  - [ ] [env] Implement `adversaries/scripted_phase.py`: `ScriptedPhaseOpponent(schedule, per_phase_policies)` — must reproduce the existing Phase VII RPS phase-switch behavior when configured equivalently (regression target for VII-B-26). Acceptance: under the existing RPS Phase VII config, action stream matches stored fixture within tolerance → env-builder  (spec §7.2, §12.2 regression)
VII-B-12.  - [ ] [env] Implement `adversaries/finite_memory_best_response.py` per spec §7.3: estimate agent empirical action distribution over last `m`, compute best response, apply inertia `λ`, optional softmax with `temperature`. Parameters via constructor (no globals). Acceptance: known-policy unit-test on a 2×2 matching pennies fixture matches hand-computed best response → env-builder  (spec §7.3)
VII-B-13.  - [ ] [env] Implement `adversaries/finite_memory_fictitious_play.py` per spec §7.4: best response to empirical opponent frequencies over last `m`, optional softmax. Acceptance: deterministic given seed; matches reference on a Shapley fixture → env-builder  (spec §7.4)
VII-B-14.  - [ ] [env] Implement `adversaries/smoothed_fictitious_play.py` per spec §7.5: quantal/logit best response `π(a) ∝ exp(Q_br(a)/T)` with log-sum-exp stabilization. Acceptance: numerical stability test at large `|Q_br|` (no overflow) → env-builder  (spec §7.5)
VII-B-15.  - [ ] [env] Implement `adversaries/regret_matching.py` per spec §7.6: cumulative-regret tracker, full-information variant (counterfactual payoffs from matrix) and realized-payoff-only variant. Probability normalization step is mandatory. Acceptance: regret vector strictly nonnegative after `max(.,0)`; row sums to 1 → env-builder  (spec §7.6)
VII-B-16.  - [ ] [env] Implement `adversaries/finite_memory_regret_matching.py` per spec §7.7: regrets computed over last `m` observations only (creates endogenous support shifts). Acceptance: support-shift event count is nonzero on a 1k-step Shapley fixture → env-builder  (spec §7.7)
VII-B-17.  - [ ] [env] Implement `adversaries/hypothesis_testing.py` per spec §7.8 (PRIORITY adversary): hypothesis hold + best-response, test window `s`, tolerance `τ`, search phase `search_len`, hypothesis resample. Must log `model_rejected / search_phase / hypothesis_id / hypothesis_distance` into the `info()` block. Acceptance: rejection trigger fires when empirical distance > τ on a hand-crafted history → env-builder  (spec §7.8, §12.1 hypothesis rejection trigger)
VII-B-18.  - [ ] [test] Unit tests `tests/adaptive_beta/strategic_games/test_adversaries.py`: deterministic behavior under fixed seed for every adversary in VII-B-10..17; regret-matching probability normalization; hypothesis rejection trigger; finite-memory window correctness. Acceptance: pytest green → test-author  (spec §12.1)  Blocked by VII-B-9..17.

### M4 — games (expands TaskCreate #5)

Game tasks depend on `MatrixGameEnv` (VII-B-6) and `registry.py` (VII-B-7); each
game registers its adversaries from M3. Soda is DEFERRED at the bottom.

VII-B-19.  - [ ] [env] Implement `games/matching_pennies.py` per spec §6.1: 2×2 zero-sum payoff, action labels {Heads=0, Tails=1}, register adversaries `stationary, finite_memory_best_response, regret_matching, hypothesis_testing`. Acceptance: payoff matrix matches §6.1; registry entry present → env-builder  (spec §6.1)
VII-B-20.  - [ ] [env] Implement `games/rps.py` per spec §6.5: reuse the existing Phase VII RPS payoff structure but make the adversary pluggable from the strategic registry; register `finite_memory_best_response, finite_memory_regret_matching, smoothed_fictitious_play, hypothesis_testing` (NOT self-play — DEFERRED per VII-B-39). Acceptance: with `scripted_phase` adversary, behavior matches the existing Phase VII RPS env (regression fixture) → env-builder  (spec §6.5, §12.2)
VII-B-21.  - [ ] [env] Implement `games/shapley.py` per spec §6.2: standard 3×3 Shapley cycle game (payoff convention documented in module docstring + config); register `fictitious_play, smoothed_fictitious_play, finite_memory_fictitious_play, regret_matching`. Acceptance: payoff documented in module docstring; pathwise non-convergence reproducible under FP on a 1k-step run → env-builder  (spec §6.2)
VII-B-22.  - [ ] [env] Implement `games/rules_of_road.py` per spec §6.3: 2×2 coordination game with Left/Right actions, tremble probability `ε`, optional payoff bias variant; register `hypothesis_testing, finite_memory_best_response`, plus inertia adversary (use `finite_memory_best_response` with `λ>0`). Acceptance: tremble parameter alters action stream; coordination payoff matches §6.3 example → env-builder  (spec §6.3)
VII-B-23.  - [ ] [env] Implement `games/asymmetric_coordination.py` per spec §6.4: 2×2 asymmetric coordination with one risk-dominant and one payoff-dominant equilibrium; register `finite_memory_fictitious_play, smoothed_best_response, inertia` (use existing FMBR with `λ>0` for inertia). Acceptance: equilibrium structure matches §6.4 → env-builder  (spec §6.4)
VII-B-24.  - [ ] [test] Unit tests `tests/adaptive_beta/strategic_games/test_games.py`: payoff matrix indexing per game; registry round-trip for all five games × declared adversaries; reset/step determinism per game. Acceptance: pytest green → test-author  (spec §12.1)  Blocked by VII-B-19..23.
VII-B-25.  - [ ] [test] Regression test `tests/adaptive_beta/strategic_games/test_rps_regression.py`: scripted-phase RPS via `games/rps.py` matches the existing Phase VII RPS implementation within numerical tolerance for a fixed seed, identical schedule, and identical episode count. Acceptance: pytest green → test-author  (spec §12.2)  Blocked by VII-B-20.
VII-B-26.  - [ ] [test] Operator-reuse regression: import-graph audit script asserting `experiments/adaptive_beta/strategic_games/**/*.py` only imports operator math from `src/lse_rl/operator/tab_operator.py` — no inlined logaddexp / expm1 reimplementations. Acceptance: pytest green; CI-friendly grep-based check → test-author  (spec §2.1, §12.2)

### M5 — metrics + logging + configs (expands TaskCreate #6)

VII-B-27.  - [ ] [logging] Implement `metrics.py`: minimum required metrics per spec §9.3 — `rolling_policy_entropy, policy_total_variation, support_shift_count, external_regret, search_phase_return, stable_phase_return, post_rejection_recovery_time, miscoordination_rate, coordination_rate`. Pure NumPy; consumes `GameHistory` + `info()` blocks. Acceptance: each metric has a unit test with hand-computed reference value → experiment-runner  (spec §9.3 minimum-implementation list)
VII-B-28.  - [ ] [logging] Logging schema additive patch: extend the existing `logging_callbacks.py` episode/transition rows with the spec §13 fields (`game, adversary, opponent_policy_entropy, policy_total_variation, support_shift, model_rejected, search_phase, phase, memory_m, inertia_lambda, temperature, tau, adversary_info_json`). Use the additive recommendation from VII-B-3; do not fork the runner. Acceptance: dev-run smoke writes both rows with all required columns + a schema header → experiment-runner  (spec §13)
VII-B-29.  - [ ] [test] Logging schema test `tests/adaptive_beta/strategic_games/test_logging_schema.py`: a 100-episode hypothesis-testing × matching-pennies smoke run produces episode CSV + transition parquet with every spec §13 column populated (no NaN-only fields). Acceptance: pytest green → test-author  (spec §12.1 event logging schema)  Blocked by VII-B-28.
VII-B-30.  - [ ] [infra] Write `configs/stage_B2_dev.yaml` exactly per spec §11.1: games {matching_pennies, strategic_rps, shapley, rules_of_road}, adversaries {finite_memory_best_response, finite_memory_regret_matching, hypothesis_testing}, methods {vanilla, fixed_positive, fixed_negative, adaptive_beta_clipped, adaptive_sign_only}, seeds {0,1,2}, episodes 1000 → 180 runs. **Self-play, Soda, Stage B2-Stress NOT in this matrix.** Acceptance: yaml parses; matrix multiplies to 180; manifest-completeness test passes → experiment-runner  (spec §11.1, branch directive)
VII-B-31.  - [ ] [infra] Write `configs/stage_B2_main.yaml` template (final game/adversary/method picks filled in by promotion gate VII-B-35): seeds 0..9, episodes 10 000, methods = core 5 from §11.2 (optional ablations magnitude-only/no-clip flagged but commented out). Acceptance: yaml parses; placeholder for promoted (game,adversary) settings is explicit → experiment-runner  (spec §11.2)
VII-B-32.  - [ ] [test] Method-matrix manifest-completeness test `tests/adaptive_beta/strategic_games/test_manifest_completeness.py`: dry-run the dev config and assert the (game, adversary, method, seed) Cartesian fanout has 180 entries, no duplicates, no missing combinations. Acceptance: pytest green → test-author  (spec §12.1 method matrix manifest completeness)  Blocked by VII-B-30.

### M6 — verifier gate after implementation (TaskCreate #7)

VII-B-33.  - [ ] [audit] **VERIFIER (after implementation):** run `tests/adaptive_beta/strategic_games/` (VII-B-5, 8, 18, 24, 25, 26, 29, 32) + smoke run (1 game × 1 adversary × `vanilla` × 100 eps). Confirm operator math is shared (VII-B-26), logging schema is complete (VII-B-29), no silent run drops, no operator duplication. Acceptance: all tests green; smoke-run artifacts present → verifier  (spec §12, §18 verifier-after-implementation)  Blocks Stage B2-Dev.

### M7 — Stage B2-Dev run + promotion gate (TaskCreate #8 + #9)

VII-B-34.  - [ ] [infra] Stage B2-Dev dispatch: run `configs/stage_B2_dev.yaml` via `run_experiment.py`. 180 runs total. Aggregate outputs to `results/adaptive_beta/strategic/processed/`. Every run accounted for in the manifest (no silent drops per spec §2.3). Acceptance: 180 manifest entries; failures logged with stack trace → experiment-runner  (spec §2.3, §11.1)  Blocked by VII-B-33.
VII-B-35.  - [ ] [analysis] Stage B2-Dev summary `results/adaptive_beta/strategic/stage_B2_dev_summary.md`: per-(game, adversary) paired-mean adaptive-β-clipped vs vanilla on AUC, recovery, mechanism (alignment, d_eff) and event-aligned signals; **promotion criteria per spec §11.1**: (a) adaptive-β improves AUC over vanilla on paired seeds in ≥1 (game, adversary) setting, (b) no clipped adaptive-β divergence, (c) mechanism/event metrics directionally support the interpretation, (d) effect not solely from a trivial stationary opponent. The memo must explicitly name the promoted (game, adversary) pair(s) (1–3) for Stage B2-Main, OR declare promotion failure. Acceptance: memo committed; promotion decision unambiguous → plotter-analyst  (spec §11.1 promotion criteria)
VII-B-36.  - [ ] [audit] **VERIFIER (after Dev):** independently re-aggregate dev outputs from raw artifacts; confirm VII-B-35 promotion decision is consistent with raw paired-seed deltas; confirm clipped adaptive-β did not diverge; confirm mechanism diagnostics direction. Acceptance: verifier sign-off memo `results/adaptive_beta/strategic/stage_B2_dev_verifier.md` → verifier  (spec §18 verifier-after-Dev)  Blocked by VII-B-35.
VII-B-37.  - [ ] [analysis] **NEGATIVE-RESULT BRANCH (spec §19):** if VII-B-35 promotion criteria are NOT met, write `results/adaptive_beta/strategic/negative_result_report.md` documenting (i) which (game, adversary) settings were tested, (ii) why each failed the four promotion criteria, (iii) honest interpretation (do not bury the result), (iv) recommendation = no Stage B2-Main, no paper update. **STOP — do not run VII-B-38..43.** Acceptance: report committed; orchestrator halts the autonomous run → plotter-analyst  (spec §19 honest reporting of weak/negative results)  Blocked by VII-B-35.

### M8 — Stage B2-Main run (TaskCreate #10) — only if VII-B-35 promotes

VII-B-38.  - [ ] [infra] Finalize `configs/stage_B2_main.yaml`: fill promoted (game, adversary) pairs from VII-B-35 into the placeholder; lock methods = {vanilla, fixed_positive, fixed_negative, adaptive_beta_clipped, adaptive_sign_only} per spec §11.2; 10 seeds; 10 000 episodes. Optional ablations only if compute budget allows. Acceptance: yaml parses; matrix size documented in memo → experiment-runner  (spec §11.2)  Blocked by VII-B-36.
VII-B-39.  - [ ] [infra] Stage B2-Main dispatch: run `configs/stage_B2_main.yaml`. Every (game, adversary, method, seed) accounted for in manifest. Acceptance: manifest complete; failures logged → experiment-runner  (spec §11.2, §2.3)  Blocked by VII-B-38.
VII-B-40.  - [ ] [plot] Event-aligned figures (HIGH PRIORITY per spec §10): `results/adaptive_beta/strategic/figures/event_aligned_return.pdf`, `event_aligned_beta.pdf`, `event_aligned_effective_discount.pdf`, `opponent_entropy.pdf`. Anchor events: model rejection, search-phase start, support shift, tremble. Acceptance: each PDF regenerable from `processed/`; spec §10 paths exact → plotter-analyst  (spec §10)  Blocked by VII-B-39.
VII-B-41.  - [ ] [plot] Stage B2-Main figure suite per spec §16: learning curves (mean ± SE), AUC paired-difference vs vanilla, β trajectory around adversary events, effective-continuation trajectory, opponent policy entropy + support shifts, strategic-metric table (regret, cycling amplitude, policy TV). Save under `results/adaptive_beta/strategic/figures/` and `tables/`. Acceptance: figures + tables regenerable; CSV + TeX outputs for tables → plotter-analyst  (spec §16)  Blocked by VII-B-39.
VII-B-42.  - [ ] [analysis] Statistical reporting per spec §15: paired-bootstrap 95% CIs, paired difference vs {vanilla, fixed-positive, fixed-negative}; primary endpoint = AUC; secondary = recovery time, catastrophic episodes, drawdown, mechanism. Save numerics to `results/adaptive_beta/strategic/processed/paired_differences.parquet`. Acceptance: every CI uses paired seeds; AUC primacy enforced (no superiority claim from final return alone) → plotter-analyst  (spec §15)  Blocked by VII-B-39.
VII-B-43.  - [ ] [analysis] Stage B2-Main summary `results/adaptive_beta/strategic/stage_B2_main_summary.md`: run matrix, missing/failed runs, metrics table, paired comparisons vs vanilla / fixed-positive / fixed-negative, mechanism diagnostics, explicit recommendation per spec §17 (main-paper update / appendix-only / no update). Acceptance: memo answers the six final-deliverable questions in spec §22 → plotter-analyst  (spec §14, §17, §22)  Blocked by VII-B-40, 41, 42.

### M9 — review gate (TaskCreate #11)

VII-B-44.  - [ ] [review] **REVIEW (after Main):** dispatch the `/lse:review`-style background gate per branch directive (NOT user-only `/codex:*`). Focus string for the reviewer: *"Challenge whether the strategic adversaries are genuinely endogenous (spec §18 review focus 1); whether mechanism evidence is nontrivial and not driven by stationary or scripted opponents (§18 focus 2 + §11.1 criterion d); whether runs are complete with no silent drops (§2.3, §18 focus 3); whether statistical claims are paired and seed-consistent (§15, §18 focus 4); whether weak/mixed results are reported honestly (§19, §18 focus 5); whether any operator math is duplicated outside `src/lse_rl/operator/tab_operator.py` (§2.1, §18 focus 6)."* Triage findings into BLOCKER / MAJOR / MINOR / NIT entries appended to this file. Acceptance: review job complete; triage entries written → review-triage  (spec §14, §18; branch directive: lse:review-style background, exact prompt above)  Blocked by VII-B-43.
VII-B-45.  - [ ] [audit] Resolve all review BLOCKERs from VII-B-44; rerun verifier on the merged branch. Acceptance: BLOCKER == ∅; verifier green → verifier  (spec §18, AGENTS.md §5.2 step 7)  Blocked by VII-B-44.

### M10 — final memo + paper-update proposal (TaskCreate #12)

VII-B-46.  - [ ] [memo] Final recommendation memo `results/adaptive_beta/strategic/final_recommendation.md` answering the six spec-§22 questions: (1) which strategic settings produced adaptive-β gains, (2) sample-efficiency vs final-return vs recovery, (3) mechanism support, (4) any fixed-β dominance, (5) any failure-mode adversary, (6) main / appendix / no paper update. Memo must NOT edit the paper. Acceptance: all six questions answered with paired-CI numbers → plotter-analyst  (spec §22)  Blocked by VII-B-45.
VII-B-47.  - [ ] [memo] Paper-update proposal — write exactly ONE of `paper_update/main_experiment_patch.md` / `paper_update/appendix_patch.md` / `paper_update/no_update_recommendation.md` based on VII-B-46 verdict and spec §17 thresholds. Proposed text only — no edits to `paper/`. Acceptance: file present; the other two absent → plotter-analyst  (spec §17, branch directive: paper edits forbidden)  Blocked by VII-B-46.
VII-B-48.  - [ ] [review] **ADVERSARIAL-REVIEW (only if `paper_update/main_experiment_patch.md` or `paper_update/appendix_patch.md` is proposed):** dispatch `/codex:adversarial-review --background` against the proposed patch with focus *"challenge generalization claims, mechanism attribution, paired-seed validity, completeness of reported failure modes."* Triage to BLOCKER/MAJOR/MINOR/NIT. Acceptance: review complete OR skipped with documented reason (no-update branch). Conditional on VII-B-47 outcome → review-triage  (spec §18 adversarial-review trigger)  Blocked by VII-B-47.

### M11 — close-out audit (TaskCreate #13)

VII-B-49.  - [ ] [audit] Verifier final audit: confirm (a) no commits to `paper/` attributable to Phase VII-B (branch directive), (b) every (game, adversary, method, seed) triple in Dev + Main accounted for in `results/adaptive_beta/strategic/manifests/` (spec §2.3), (c) `final_recommendation.md` answers all six §22 questions, (d) `paper_update/` contains exactly one file, (e) operator-reuse import-graph still passes (VII-B-26), (f) no MushroomRL-dev edits introduced by this phase (or, if any, recorded in `tasks/lessons.md`). Acceptance: audit memo signed off; phase closeable → verifier  (spec §22, branch directive)  Blocked by VII-B-48 (or VII-B-47 if adversarial-review skipped).

### DEFERRED — do not run in this phase

VII-B-D1.  - [ ] [env] DEFERRED — do not run. Self-play adversary `adversaries/self_play.py` per spec §7.10 (variants vanilla×vanilla, adaptive×vanilla, adaptive×adaptive, adaptive×fixed_positive, adaptive×fixed_negative). Track for a follow-up phase only after Stage B2-Main consolidates. → env-builder  (spec §7.10, branch directive: DEFERRED)
VII-B-D2.  - [ ] [env] DEFERRED — do not run. `games/soda_game.py` per spec §6.6 (hidden opponent type ∈ {coordination, anti-coordination, zero-sum, biased}). Optional first-pass per spec; explicitly excluded from Dev and Main matrices on this branch. → env-builder  (spec §6.6, branch directive: DEFERRED)
VII-B-D3.  - [ ] [env] DEFERRED — do not run. `adversaries/realized_payoff_regret.py` per spec §7.9 (epsilon-experimentation, value-LR updating). Spec marks optional for first pass. → env-builder  (spec §7.9, branch directive: DEFERRED)
VII-B-D4.  - [ ] [infra] DEFERRED — do not run. `configs/stage_B2_stress.yaml` + Stage B2-Stress dispatch per spec §11.3 (memory, inertia, tau, search length, temperature, payoff noise, tremble). Only run if Stage B2-Main is strong AND user re-authorizes. → experiment-runner  (spec §11.3, branch directive: DEFERRED)

### Dependencies (blocking edges)

- M0 (VII-B-1) is informational only; non-blocking but feeds M1 decisions.
- M1 → M2: VII-B-4 (history) blocks VII-B-5; VII-B-2 scaffold blocks all subsequent module work.
- M2 → M3: VII-B-6 (matrix-game env) blocks VII-B-9..17 (adversaries integrate via the env interface) and VII-B-19..23 (games).
- M3 → M4: VII-B-9 (base adversary) blocks VII-B-10..17; VII-B-10..17 collectively unblock the games via the registry.
- M4 → M5: games (VII-B-19..23) feed metrics + logging (VII-B-27, 28); regression test VII-B-25 blocks Stage B2-Dev (VII-B-34) since RPS continuity is a §12.2 hard requirement.
- M5 → M6: VII-B-30 (Dev config) + VII-B-32 (manifest test) + VII-B-29 (logging schema) unblock VII-B-33 (verifier-after-impl).
- M6 → M7: VII-B-33 verifier gate blocks VII-B-34 (Dev dispatch).
- M7 promotion: VII-B-35 outcome forks: pass → VII-B-36 → M8; fail → VII-B-37 → STOP.
- M8 → M9: VII-B-43 (Main summary) blocks VII-B-44 (review gate).
- M9 → M10: VII-B-45 (BLOCKERs resolved) blocks VII-B-46 (final memo); VII-B-47 (paper update) gates VII-B-48 (adversarial-review conditional).
- M10 → M11: VII-B-48 (or VII-B-47 if no-update) blocks VII-B-49 (closeout audit).

### Parallelizable groups (worktree dispatch)

- **Group SP1** (after VII-B-2 scaffold + VII-B-4 history): VII-B-5 (history test), VII-B-6 (matrix-game env), VII-B-7 (registry) — three independent worktrees.
- **Group SP2** (after VII-B-9 base adversary): VII-B-10, VII-B-11, VII-B-12, VII-B-13, VII-B-14, VII-B-15, VII-B-16, VII-B-17 — eight independent adversary worktrees. Each is an atomic single-file implementation.
- **Group SP3** (after VII-B-6 + VII-B-7 + Group SP2): VII-B-19, VII-B-20, VII-B-21, VII-B-22, VII-B-23 — five independent game worktrees.
- **Group SP4** (after Group SP3): VII-B-24 (game tests), VII-B-25 (RPS regression), VII-B-26 (operator-reuse audit), VII-B-27 (metrics), VII-B-28 (logging schema patch) — five independent worktrees. VII-B-18 (adversary tests) can join SP4 since it depends only on VII-B-9..17.
- **Group SP5** (after VII-B-39 Main dispatch): VII-B-40 (event-aligned figures), VII-B-41 (figure suite), VII-B-42 (statistical reporting) — three independent worktrees feeding VII-B-43 (Main summary).

### Critical path (M0 → final memo)

VII-B-1 → VII-B-2 → VII-B-4 → VII-B-6 → VII-B-9 → VII-B-17 (hypothesis-testing, priority adversary per §21.1) → VII-B-19 (matching pennies, priority game per §21.1) → VII-B-27 → VII-B-28 → VII-B-30 → VII-B-32 → VII-B-33 (verifier) → VII-B-34 (Dev dispatch) → VII-B-35 (promotion) → VII-B-36 (verifier-after-Dev) → VII-B-38 → VII-B-39 (Main dispatch) → VII-B-43 (Main summary) → VII-B-44 (review) → VII-B-45 → VII-B-46 (final memo) → VII-B-47 (paper-update proposal). Other game/adversary tasks fan out off this spine via SP2/SP3/SP4.

### Open questions (planner — please surface to user)

- **OQ-1.** Spec §6.2 (Shapley) defers payoff convention to "code comments and config" — multiple Shapley conventions exist (e.g., the canonical 1964 matrix vs Hofbauer-Sigmund variants). Which convention should the implementation pin? (Planner default if unanswered: the 1964 Shapley matrix as documented in Fudenberg–Levine, recorded in module docstring.)
- **OQ-2.** Spec §11.1 Dev matrix lists `strategic_rps` as a game name, but the existing Phase VII RPS env uses scripted phases. Should Stage B2-Dev's `strategic_rps` rows pair `rps` with the three §11.1 adversaries (FMBR, FM-RM, hypothesis-testing) and EXCLUDE the scripted-phase variant from Dev? (Planner default if unanswered: yes — exclude scripted-phase from Dev to preserve §11.1 promotion criterion d "effect is not solely from a trivial stationary opponent"; keep scripted-phase only as the regression-fixture target in VII-B-25.)
- **OQ-3.** Spec §10 lists four event-aligned figures with exact paths, but does not specify a window (±N episodes) around the anchor event. (Planner default if unanswered: ±50 episodes for episode-level metrics, ±20 transitions for transition-level metrics — to be confirmed in VII-B-40.)
- **OQ-4.** Spec §11.2 says "limited ablations" optional if time. Given the ≤20-min wall-clock budget, should ablations (`adaptive_magnitude_only`, `no-clip`) be included in `configs/stage_B2_main.yaml` or hard-skipped? (Planner default if unanswered: hard-skip in the initial Main dispatch; add as a follow-up only if VII-B-43 leaves budget headroom.)
- **OQ-5.** Branch directive says the review gate is `/lse:review`-style (NOT `/codex:*`). The slash command `/lse:review` is not in the AGENTS.md §4 dispatch table or under `.claude/commands/lse/`. Should the planner-recap memo (VII-B-1) propose a concrete review-job spec (model, prompt template, output path) or block on the user defining `/lse:review` first?

## Phase VII-B Review Triage (2026-04-26)

Internal review pass against `tasks/phase_VII_B_strategic_learning_coding_agent_spec.md`
+ AGENTS.md §4 / §5.2 + parent `docs/specs/phase_VII_adaptive_beta.md`.
Branch: `phase-VII-B-strategic-2026-04-26`. Verdict: **NO UPDATE**
(fixed_negative dominates adaptive_beta on Shapley × HypTest auc_full,
CI excludes 0). Severity bands: BLOCKER (must fix before merge to main),
MAJOR (fix before merge but does not invalidate verdict), MINOR
(deferrable), NIT (trivial).

### BLOCKER

- [ ] [BLOCKER] [test] Stage B2-Main placeholder smoke test fails — analyst overwrote the placeholder live YAML, breaking
      `tests/adaptive_beta/strategic_games/test_runner_smoke.py::test_runner_rejects_unfilled_main_config_placeholder`.
      Concrete acceptance criterion: the test must (a) construct a fixture YAML containing `${promoted_games}` / `${promoted_adversaries}`
      placeholder strings programmatically (or read from a frozen fixture under `tests/adaptive_beta/strategic_games/fixtures/`)
      rather than reading the live `experiments/adaptive_beta/strategic_games/configs/stage_B2_main.yaml`,
      AND a separate test must assert the live `stage_B2_main.yaml` is dispatchable (no placeholder remaining); both
      must pass under `pytest -m "not slow"`. Confirmed-failing today: `python3 -m pytest tests/adaptive_beta/strategic_games/test_runner_smoke.py::test_runner_rejects_unfilled_main_config_placeholder` → `Failed: DID NOT RAISE`.
      → test-author  (spec §11.2 / §13.2; refs `tests/adaptive_beta/strategic_games/test_runner_smoke.py:143-152` + `experiments/adaptive_beta/strategic_games/configs/stage_B2_main.yaml:25-40`)

- [ ] [BLOCKER] [infra] Stage B2-Main config was edited in-place destroying the placeholder contract.
      Spec §11.2 + the YAML's own header comment promised `${promoted_*}` placeholders that the promotion gate would fill.
      Concrete acceptance criterion: restore `experiments/adaptive_beta/strategic_games/configs/stage_B2_main.yaml` to a placeholder template form (lines 29-40 → `games: ${promoted_games}` / `adversaries: ${promoted_adversaries}`), AND emit the resolved-promoted config to a separate file
      (`experiments/adaptive_beta/strategic_games/configs/stage_B2_main_resolved_2026-04-26.yaml`)
      that records the promoted Cartesian product as a stage artifact. `paper_update/no_update_recommendation.md` must cite the resolved file, not the template.
      → experiment-runner  (spec §11.2 + `experiments/adaptive_beta/strategic_games/configs/stage_B2_main.yaml:7-13` header comment which itself documents the in-place-overwrite contract)

- [ ] [BLOCKER] [infra] Phase VII-B work is entirely uncommitted — `phase-VII-B-strategic-2026-04-26` and `phase-VII-overnight-2026-04-26` both point at `656e7cbf`. Concrete acceptance: stage all VII-B artifacts (`experiments/adaptive_beta/strategic_games/`, `tests/adaptive_beta/strategic_games/`, `scripts/figures/phase_VII_B/`, `results/adaptive_beta/strategic/`, `paper_update/no_update_recommendation.md`, `tasks/phase_VII_B_strategic_learning_coding_agent_spec.md`, `docs/phase_VII_adaptive_beta_summary.md`) into one or more commits on the VII-B branch BEFORE running `/codex:review --base phase-VII-overnight-2026-04-26 --background` — Codex reviews the diff, and an empty diff means an empty review. `git diff phase-VII-overnight-2026-04-26..HEAD --stat` currently produces no output. → experiment-runner  (AGENTS.md §5.2 step 3; `git rev-parse phase-VII-{overnight,B-strategic}-2026-04-26` both = `656e7cbf`)

### MAJOR

- [ ] [MAJOR] [infra] Stage B2-Dev manifest pollution — `results/adaptive_beta/strategic/raw/dev/manifest.json` contains 181 records (180 expected) because `(matching_pennies, finite_memory_best_response, vanilla, seed=0)` has both an `n_episodes=5` smoke duplicate and an `n_episodes=1000` real run. The on-disk `metrics.npz` is the latest 1000-ep file, so `aggregate.py`'s `rglob("metrics.npz")` lookup is correct — but the manifest itself is wrong, and any consumer that joins via the manifest (e.g. the audit script in M11) will silently double-count. Concrete acceptance: either (a) fix the runner to dedupe `(game, adversary, method, seed_id)` keeping latest `started_at` on append, OR (b) write a one-shot manifest cleaner that rewrites `manifest.json` in place removing the stale 5-episode record AND add a runner-level guard that raises `ValueError` if a duplicate cell key would be appended. Test fixture: append two records with same `cell_id` and assert dedup behaviour. → experiment-runner  (spec §13 manifest contract; refs `results/adaptive_beta/strategic/raw/dev/manifest.json` + `experiments/adaptive_beta/strategic_games/run_strategic.py:239-262`)

- [ ] [MAJOR] [code] `run_strategic.run_one_cell` duplicates the Phase VII-A `run_experiment.run_one` episode-loop body verbatim (lines 686-845: `agent.begin_episode → env.reset → action loop → ep_logger.record → strategic_logger.record_episode_strategic`). Phase VII-A's runner already implements 90% of this; only the strategic-schema augmentation is novel. If Phase VII-A's operator math, ε-greedy, or schedule contract changes, the VII-B runner will silently desync. Concrete acceptance: refactor to delegate the per-episode work to a shared helper (e.g. `experiments.adaptive_beta.run_experiment._run_one_episode(agent, env, ep, ep_logger, tr_logger, eps_fn) → episode_diag_dict`) and have `run_strategic` wrap that helper with strategic-schema bookkeeping (auc_running, opp_dist, support_shift, model_rejected, search_phase). The VII-A runner's behaviour must remain byte-identical (regression-test the Phase VII-A Stage A summary numbers). → algo-implementer  (spec §2.2 "do not modify stable Phase III–VI machinery unless strictly necessary" — this PR introduces a duplicate path it was supposed to reuse; refs `experiments/adaptive_beta/strategic_games/run_strategic.py:686-845` vs `experiments/adaptive_beta/run_experiment.py`)

- [ ] [MAJOR] [analysis] Single-game Stage B2-Main scope (`shapley` × {FMRM, HypTest}) is below the spec §17 "≥ 2 strategic settings" main-paper-update bar AND below the appendix-only bar of "one strong setting + mixed others". The analyst dropped the Dev-promoted `(matching_pennies, FMRM)` cell to maintain a Cartesian product with the existing adversaries; this is hygienic but limits the generalization claim. Concrete acceptance: either (a) extend Stage B2-Main with `matching_pennies × FMRM` × 10 seeds × 10k episodes (≈ same wall-clock cost) and re-run the verdict, OR (b) write a `paper_update/scope_limit_note.md` companion to `no_update_recommendation.md` that explicitly states the Cartesian-product hygiene cost in spec-§17 terms. (b) is acceptable for this phase but the gap MUST be visible to the next reviewer. → experiment-runner + plotter-analyst (split: runner extends Main; plotter writes scope-limit note if (b) is chosen)  (spec §11.2 + §17; refs `results/adaptive_beta/strategic/stage_B2_dev_summary.md:5-12` Cartesian-product note)

- [ ] [MAJOR] [analysis] Strategic RPS regression silently demoted. Stage B2-Dev shows adaptive_beta UNDER-PERFORMS vanilla on `strategic_rps` × all three endogenous adversaries (Δ AUC = -744, -415, -703 at n=3 seeds; mechanism evidence `align∈{0.74, 0.54, 0.56}` and `d_eff∈{0.62, 0.88, 0.86}`). This contradicts the Phase VII-A headline RPS gain on scripted phase opponents. Spec §2.4 / §17 + spec §18 require honest reporting of failure modes; the Main summary (§8) and the no-update memo MENTION it as a follow-up but neither escalates the implication that the **Phase VII-A RPS narrative may be adversary-specific to scripted-phase opponents**. Concrete acceptance: rerun `strategic_rps × {finite_memory_regret_matching, hypothesis_testing}` × 10 seeds × 10k episodes (Stage B2-Main–equivalent budget) AND extend `paper_update/no_update_recommendation.md` with a `## Phase VII-A scope hedge` section that either (i) reproduces the regression at the higher seed budget and recommends scoping the paper's RPS claim to scripted opponents, or (ii) shows the regression was an n=3 seeds artifact. → experiment-runner + plotter-analyst  (spec §17 + §18 honest-failure rule; refs `results/adaptive_beta/strategic/stage_B2_main_summary.md:163-165` + `stage_B2_dev_summary.md:66-77`)

- [ ] [MAJOR] [logging] `oracle_action` is never populated in `MatrixGameEnv.step`'s `info` dict — line 417-428 omits it. Downstream `tr_logger.record(... oracle_action=step_info.get("oracle_action", None))` always logs `None`, and the `regret` column in `episode_to_row(...)` is forced to `nan` (see `run_strategic.py:754`). Spec §9.1 lists `regret` as a core performance metric and §13 lists `regret` in the strategic-schema columns; spec §9.3 lists `external_regret` as a strategic-learning metric. This silently breaks two metrics. Concrete acceptance: for each game (`strategic_rps`, `shapley`, `matching_pennies`, `rules_of_road`, `asymmetric_coordination`), either (a) define an oracle as the empirical-best-response to the opponent's empirical play over the most-recent window AND populate `info["oracle_action"]`, OR (b) compute an `external_regret` per-episode that does not require an oracle (e.g. `max_a payoff_agent[a, .] @ p_opp_emp - mean_realized_reward`). Plug the result into the strategic-schema log so Stage B2-Main's `Strategic-Metric Table` row no longer reports `regret = "--"`. → algo-implementer  (spec §9.1 + §9.3 + §13; refs `experiments/adaptive_beta/strategic_games/matrix_game.py:417-428` + `run_strategic.py:754` + `stage_B2_main_summary.md:101-111` `regret` column = `--`)

- [ ] [MAJOR] [code] `HypothesisTestingAdversary.act()` `search_phase` reporting has an off-by-one (the test author flagged it as cosmetic). `_search_remaining` is decremented at the END of `act()`, so on the round where `search_remaining == 1` going in, the post-act `info()` call reports `search_phase = False` even though that round's action was the last uniform-random search action. The substantive contract (uniform-random play for `search_len` rounds) holds; only the reported state-machine flag is stale by one round. Test
      `test_hypothesis_search_phase_length_equals_search_len` already accepts both `n_true == search_len` and `n_true == search_len - 1`, papering over the bug. Concrete acceptance: either (i) move the decrement to the BEGINNING of `act()` so `info()` reads `search_phase = True` for exactly `search_len` consecutive post-act snapshots, OR (ii) keep current behaviour but add a `search_round_index` field to `info()` that disambiguates "currently in search" vs "this was the last search round" — and tighten the test to assert exactly one of the two interpretations rather than accepting either. → algo-implementer  (spec §7.8; refs `experiments/adaptive_beta/strategic_games/adversaries/hypothesis_testing.py:233-245` + `tests/adaptive_beta/strategic_games/test_adversaries.py:316-357` lenient assertion)

- [ ] [MAJOR] [analysis] Stage B2-Main `recovery_time` metric is degenerate on Shapley × {FMRM, HypTest}. Per the Main memo §9 methodological note: support_shift fires on ≈97% of FMRM episodes and ≈78% of HypTest episodes (because TV against the previous episode's empirical opponent dist is `> 0.1` whenever a Shapley cycle realises differently across episodes); the spec-§7.1 wording "first episode the smoothed return reaches 80% of asymptotic mean *following a support_shift event*" is satisfied by essentially every episode, so `recovery_time` collapses to a global learning-speed metric and not to a regime-shift recovery metric. The Main figures still ship `recovery_time_main.pdf` as if it were a recovery panel. Concrete acceptance: either (a) re-define `support_shift` with a stricter threshold (e.g. TV `> 0.3`, gating the average to one shift per ~50 episodes) and re-run the recovery analysis, OR (b) replace `recovery_time` on shapley cells with a "first-episode-to-reach-80%-of-asymptote" metric labeled `learning_speed_episodes`, and update `recovery_time_main.pdf` plus the no-update memo to say `recovery is not measurable on this game family with the current support_shift definition`. → plotter-analyst  (spec §7.1 + §10 figure 2; refs `scripts/figures/phase_VII_B/stage_B2_main_analysis.py:170-220` + `stage_B2_main_summary.md:170` methodological note)

- [ ] [MAJOR] [test] `RegretMatching(mode='realized_payoff')` violates the §7.6 second mode's spirit. The `observe` path in this mode reads `cf = self._payoff_opponent[a, :]` (the FULL row of counterfactuals) before computing regret — even though the docstring promises "Q estimates updated through experimentation only". This is fine for the spec literal which only requires "support both modes"; but the realized-payoff branch is observationally indistinguishable from the full-info branch for a single-row update. Concrete acceptance: either (a) rename the mode `realized_payoff_full_row` and gate the truly experimentation-driven variant behind `RealizedPayoffRegret` (the §7.9 stub), OR (b) refactor the realized-payoff branch in `RegretMatching.observe` to use ONLY the realised opponent reward (not the full row), and add a unit test asserting that the regret update never reads `payoff_opponent[a, j]` for `j != b`. The Stage B2-Dev / Main matrices do not exercise this mode, so no headline number changes either way — but the spec contract is currently performative. → algo-implementer + test-author  (spec §7.6 + §7.9; refs `experiments/adaptive_beta/strategic_games/adversaries/regret_matching.py:165-178`)

### MINOR

- [ ] [MINOR] [analysis] Stage B2-Main ablations `adaptive_magnitude_only` and `adaptive_beta_no_clip` deferred per OQ-4. Spec §11.2 lists them as "optional if time"; the no-update verdict makes them moot for the paper, but they are still part of the §22 question 5 ("did any adversary expose a failure mode") audit. Concrete acceptance: either (a) write a one-paragraph `Ablation deferral note` in `paper_update/no_update_recommendation.md` explaining why the no-clip and magnitude-only ablations were not run AND that the no-clip-divergence hypothesis is not falsifiable on this game family without running them, OR (b) add a small ablation pass (3 seeds × 5k episodes × 2 methods × 2 cells = 12 runs, low cost) and report the divergence-event count + final-return + AUC. → plotter-analyst  (spec §11.2; refs `tasks/todo.md` OQ-4 + `experiments/adaptive_beta/strategic_games/configs/stage_B2_main.yaml:15-17`)

- [ ] [MINOR] [analysis] Stage B2-Dev summary §6 "Selection-Bias Acknowledgement" notes that matching_pennies × FMRM cleared the strict gate but was dropped from Main for Cartesian-product hygiene. The drop is justified (keeping it would force matching_pennies × HypTest into Main, which gate-failed) — but the §22 question 1 in the no-update memo does NOT count `matching_pennies × FMRM` as a Dev-promoted-but-not-Main-tested cell. Concrete acceptance: extend `paper_update/no_update_recommendation.md` with one row in the headline table noting `(matching_pennies, FMRM): Dev-promoted, dropped for Cartesian hygiene, untested at n=10`. → plotter-analyst  (spec §17 + Dev memo §7; refs `results/adaptive_beta/strategic/stage_B2_dev_summary.md:127-134`)

- [ ] [MINOR] [logging] The strategic-schema CSV (`episodes_strategic.csv`) was added in parallel to the VII-A `episodes.csv`, both written from the same per-episode loop. The VII-A `episodes.csv` does NOT carry the strategic columns (`opponent_policy_entropy`, `policy_total_variation`, `support_shift`, `model_rejected`, `search_phase`, `memory_m`, `inertia_lambda`, `temperature`, `tau`), so any consumer that loads only `episodes.csv` silently misses them. Concrete acceptance: add a top-of-file comment to `episodes.csv` stating "see episodes_strategic.csv for VII-B columns", OR rename to `episodes_legacy.csv` and emit a unified `episodes.csv` with both schemas. → experiment-runner  (spec §13 + spec parent §15; refs `experiments/adaptive_beta/strategic_games/run_strategic.py:847-860` + `experiments/adaptive_beta/logging_callbacks.py`)

- [ ] [MINOR] [test] No test asserts that `support_shift = (policy_tv > 0.1)` (the threshold hard-coded at `run_strategic.py:801`) is the operative definition. A regression that flipped the constant or the comparator would silently drift the `support_shift_count` column. Concrete acceptance: add a test in `test_logging.py` or `test_metrics.py` that constructs two episodes with hand-crafted opponent policies of TV distance 0.05 and 0.15 and asserts `support_shift = False` then `True`. → test-author  (spec §13; refs `experiments/adaptive_beta/strategic_games/run_strategic.py:801`)

- [ ] [MINOR] [code] `MatrixGameEnv.reset` does NOT reset the adversary on episodes ≥ 1 — by design (§7 endogenous adversaries accumulate across episodes). However, the comment at line 327-330 mixes "reset adversary on first episode" with "do not clobber rolling state". A spy that reads `_episode_index` between episodes can't tell whether a re-seed call would hurt or help. Concrete acceptance: add a docstring example to `MatrixGameEnv.reset` showing two equivalent re-instantiations (fresh adversary + first reset() vs accumulated adversary + N-th reset()) with the explicit invariant `adversary_state(0) ≠ adversary_state(N)`. → algo-implementer  (spec §6.1 + §7; refs `experiments/adaptive_beta/strategic_games/matrix_game.py:311-358`)

- [ ] [MINOR] [test] No test exercises `dispatch_from_config` on the Stage B2-Main *resolved* config (the live YAML with the promoted Cartesian product). The smoke test currently only covers Stage B2-Dev. Concrete acceptance: add a small test that loads `stage_B2_main.yaml` (after BLOCKER above is resolved and a placeholder template is restored, OR load the new `_resolved_2026-04-26.yaml`), shrinks `episodes` to 3, dispatches `--limit-cells 1`, and asserts a completed manifest record on the Shapley × FMRM × adaptive_beta cell. → test-author  (spec §11.2; refs `tests/adaptive_beta/strategic_games/test_runner_smoke.py`)

### NIT

- [ ] [NIT] [code] `experiments/adaptive_beta/strategic_games/adversaries/finite_memory_best_response.py:48` — tie-tolerance epsilon hard-coded as `1e-12`; for n_actions=3 this is fine but for higher action spaces with very small payoff differences the tolerance might fold ties incorrectly. Move to a module-level constant. → algo-implementer

- [ ] [NIT] [code] `MatrixGameEnv.game_info()` returns a fresh dict every call (line 282 comment "safe to mutate"). The dict creation is cheap but called in a hot path — inline the keys at construction time and return a shallow copy if mutation safety is required. → algo-implementer

- [ ] [NIT] [code] `_GAME_CANONICAL_SIGNS` in `run_strategic.py:447-453` is a static lookup table that duplicates the `metadata['canonical_sign']` field every game module already exports. Replace with a one-line lookup `getattr(_game_module(g_name), 'metadata', {}).get('canonical_sign')` or via the registered factory's first-call introspection. → algo-implementer
