...
The experiments in this section are mechanism demonstrations for the
deployed safe weighted-LSE Bellman operator, not broad benchmarking.
We ask two specific questions.
\emph{(i)}~Can the deployed safe operator change the optimal policy at a
classical near-tie while preserving classical-objective performance?
\emph{(ii)}~Does safe clipping preserve the contraction certificate
where the raw weighted-LSE operator would leave the certified region?
All experiments use exact finite-horizon planning and exact policy
evaluation on constructed tabular MDPs aligned with the theorems.
No RL learning curves are included; the positive evidence is at the
operator level.

\subsection{Near-tie construction and metrics}
\label{sec:exp-principles}

Fix a designated contest state $x_c=(0,s_0)$ and two candidate
actions~$a_1,a_2$. The classical action gap is
$\Delta_0(x_c;\theta) = Q^*_{0,\gamma}(x_c,a_1;\theta)
- Q^*_{0,\gamma}(x_c,a_2;\theta)$,
and the normalized gap is
$\mathrm{contest\_gap\_norm}=|\Delta_0(x_c)|/R_\mathrm{scale}$,
with $R_\mathrm{scale}=\max(R_\mathrm{max},|V^*_{0,\gamma}(s_0)|,10^{-8})$.
We construct tasks so that a designated reachable contest state sits
in the strict near-tie band $\mathrm{contest\_gap\_norm}\le 10^{-2}$
and call a task \emph{promotable} when, in addition, the safe-optimal
policy differs from the classical-optimal policy either at $s_0$
(\emph{start-state flip}) or on a non-negligible fraction of the
reference occupancy
$d_\mathrm{ref}=\tfrac12 d^{\pi^*_\mathrm{cl}}+\tfrac12 d^{\pi^*_\mathrm{safe}}$
(\emph{policy disagreement}), the normalized safe value gap
$|vgn|=|V^*_\mathrm{safe}(s_0)-V^*_\mathrm{cl}(s_0)|/R_\mathrm{scale}$
is at least $5\times 10^{-3}$, and no certification violation occurs
on visited transitions.

The local continuation derivative
$d_t(r,V)=(1+\gamma)\bigl(1-\sigma(\beta_t(r-V)+\log\tfrac1\gamma)\bigr)$
is the key stagewise diagnostic: it equals $\gamma$ at $\beta_t=0$ and
exceeds $\gamma$ when the operator is nonlinearly active. The certified
stagewise bound is $\kappa_t=\gamma+\alpha_t(1-\gamma)$. We report two
clipping modes: $\texttt{binding\_clip}$, where clipping is active on a
nondegenerate fraction of visited transitions, and
$\texttt{safe\_active\_no\_distortion}$, where the raw schedule is
already certified on visited transitions so the deployed safe operator
coincides with the raw operator while both remain within the certified
region.

Table~\ref{tab:phaseVI_summary} summarizes the headline rows that
support the two positive claims developed in the next two subsections.
\input{../results/summaries/main_table.tex}

\subsection{Family A: objective-level policy improvement at classical ties}
\label{sec:exp-mechanism}

Family A is a two-branch contest: from the contest state, action $A$
leads to a single delayed terminal reward (a concentrated jackpot) and
action $B$ leads to a smooth per-step stream. We tune the tie parameter
so the two branches are classically near-indifferent and then sweep a
narrow $\varepsilon$-band around the classical tie.

\paragraph{Deterministic Family A.}
On the five deterministic Family~A promoted tasks, the safe-optimal
policy flips the start-state greedy action on every task, with
$|vgn|$ up to $7.82\times 10^{-3}$ and policy disagreement up to
$0.25$ under $d_\mathrm{ref}$. Clipping is inactive
($\texttt{safe\_active\_no\_distortion}$) because the raw schedule is
already inside the certified box, so the deployed operator and the
raw weighted-LSE operator coincide on visited transitions.

\paragraph{Stochastic Family A: classically regret-free, safe-eval better.}
On the six stochastic Family~A promoted tasks
($p_\mathrm{transit}=0.95$), exact policy evaluation under the two
Bellman objectives yields
\[
\underbrace{V^{\pi^*_\mathrm{cl}}_\mathrm{cl}(s_0) - V^{\pi^*_\mathrm{safe}}_\mathrm{cl}(s_0)}_{\text{classical-eval regret}}
\le 4.4\times 10^{-16}
\quad\text{and}\quad
\underbrace{V^{\pi^*_\mathrm{safe}}_\mathrm{safe}(s_0) - V^{\pi^*_\mathrm{cl}}_\mathrm{safe}(s_0)}_{\text{safe-eval advantage}}
\in [1.2\times 10^{-2},\,4.99\times 10^{-1}].
\]
Classical-evaluation regret is at machine precision on every task
(all six values $\le 4.4\times 10^{-16}$). Safe-evaluation advantage
is strictly positive on every task, with a mean of about
$1.56\times 10^{-1}$ and a per-task magnitude that scales with the
task's reward amplitude. \Cref{fig:familyA_dual_objective} visualizes
both sides of the comparison. This is the cleanest empirical signature
of the proposed operator: at a classical near-tie, the safe
weighted-LSE objective selects a policy that is classically regret-free
but strictly preferred by the safe recursive objective.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{../figures/main/fig_familyA_dual_objective.pdf}
\caption{\textbf{Safe weighted-LSE policy selection at a classical
near-tie.} Across six stochastic Family~A tasks, the safe-optimal
policy has machine-precision classical-evaluation regret and strictly
positive safe-evaluation advantage. The mechanism therefore lives in
the Bellman objective rather than in stochastic return noise.}
\label{fig:familyA_dual_objective}
\end{figure}

\subsection{Family C: clipping preserves the certificate that raw violates}
\label{sec:exp-safety}

Family C is a raw-stress family engineered so that a raw weighted-LSE
schedule with elevated temperature crosses the certified contraction
budget on visited transitions. The deployed safe schedule clips the
same raw signal to the certified stagewise bound $\kappa_t$. On the
$84$ promoted Family~C tasks, the safe-clipped operator's local
continuation derivative stays below $\kappa_t\in[0.95,0.96]$ on every
visited transition ($\texttt{stable\_nonlinear}$ classification); the
raw operator violates the certificate with $d_\mathrm{raw}$ reaching
$1.416$ ($\texttt{expansive\_bounded}$). On the lead task the raw
backward sweep places $V[0,s_0]$ at approximately $16.9$, while
classical and safe remain tightly grouped around $4.86$ and $4.92$
respectively (\Cref{fig:familyC_certificate}).

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{../figures/main/fig_familyC_certificate.pdf}
\caption{\textbf{Safe clipping preserves the contraction certificate.}
On the raw-stress task, the raw weighted-LSE operator places derivative
mass beyond the nonexpansive threshold, while the deployed safe
operator remains below the certified stagewise bound. The safe values
remain close to classical, whereas the raw target leaves the certified
region.}
\label{fig:familyC_certificate}
\end{figure}

The raw operator is the expressive object; the clipped operator is
the deployable object. Family~C shows why the distinction matters:
clipping preserves the contraction certificate without collapsing back
to a purely classical backup.

\subsection{Takeaway}
\label{sec:exp-takeaway}

Family~A demonstrates positive mechanism translation at a tuned
classical decision boundary: the safe weighted-LSE policy is
classically regret-free and strictly preferred under the safe
objective. Family~C demonstrates that the safety layer is both
necessary and effective: clipping preserves the certified contraction
bound on a region where the raw operator exceeds it. Taken together
the experiments support the theory --- the weighted-LSE operator is
not a universal return-improvement heuristic, it is a certified
Bellman-objective mechanism that changes temporal credit assignment
in theorem-linked cases.
...
% =====================================================================
\section{Supplementary sanity checks and calibration artifacts}
\label{app:phase_I_IV_sanity}
\label{app:design_diagnostics}
% =====================================================================

An earlier broad benchmark matrix (eight tabular families, classical
vs.\ safe algorithms) was used to verify the calibration and
certification pipeline. On that matrix the deployed clipped schedule
sits in the near-zero-$\beta$ regime on every task, so the safe
operator coincides with classical to numerical precision and the
DP/RL comparison is uninformative at the operator level. The
machinery itself is internally consistent: certification invariants
hold on every run, and the empirical effective discount
$\hat{d}_t$ matches $\gamma$ to within $10^{-4}$. The sign of $\beta$
governs whether the safe correction amplifies gains or penalizes
losses; which sign is appropriate depends on the task, and the main
text is limited to the optimistic-sign regime where the positive
objective-level claim applies. Full per-run artifacts are under
\texttt{results/weighted\_lse\_dp/} in the supplementary material.
...
% Phase VII appendix (exploratory adaptive-β credit-assignment result on
% adversarial RPS). Linked at user request 2026-04-26 after the
% phase-VII-overnight-2026-04-26 run produced a paired-bootstrap-
% significant AUC improvement (CI [+1473, +1970]) on a single env with
% mechanism evidence (alignment 0.79, d_eff 0.57 vs γ=0.95).
\input{phase_VII_appendix_draft}
